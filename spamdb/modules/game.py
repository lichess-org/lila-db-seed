...
def update_game_colls() -> list:
    ...
    if not args.no_create:
        ...
        util.bulk_write(db.crosstable2, list(crosstable.values()))
        util.bulk_write(db.matchup, list(crosstable.values()))
    return games
...
class Game:
    def __init__(self, game: dict, white: str, black: str):
        ...
        self.so = game['so']
        self.hp = binary.Binary(game['hp'])
        self.an = False
        if 'c' in game:
            self.c = binary.Binary(game['c'])
        if 'cw' in game:
            self.cw = binary.Binary(game['cw'])
        if 'cb' in game:
            self.cb = binary.Binary(game['cb'])
        if 'w' in game:
            self.w = game['w']
