...
class Token:
    def __init__(self, uid: str):
        password = env.get_password(uid)
        suffix = '-' + password if password != 'password' else ''
        token = 'lip_' + uid + suffix
        self.plain = token
        self.userId = uid
...
def _create_special_users():
    users: list[User] = []
    users.append(User('lichess', [], ['ROLE_VERIFIED'], False))
    ...
    users.append(User('broadcaster', [], ['ROLE_STUDY_ADMIN'], False))
